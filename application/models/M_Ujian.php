...
class M_Ujian extends CI_Model
{
    ...
    {
        $this->datatables->select("a.id_ujian, e.nama_guru, d.nama_kelas, a.nama_ujian, b.nama_matpel, a.jumlah_soal, CONCAT(a.tgl_mulai, ' <br/> (', a.waktu, ' Menit)') as waktu,  (SELECT COUNT(id) FROM h_ujian h WHERE h.user_id = {$id} AND h.ujian_id = a.id_ujian) AS ada");
        $this->datatables->from('m_ujian a');
        $this->datatables->join('matpel b', 'a.matpel_id = b.id_matpel');
    ...
    public function HslUjian($id, $user)
    {
    ...
    public function HslUjianById($id, $dt=false)
    {
        ...
        $this->$db->select('d.id, a.name, b.nama_kelas, d.jml_benar, d.nilai');
        $this->$db->from('user a');
        $this->$db->join('kelas b', 'a.kelas_id=b.id_kelas');
        $this->$db->join('h_ujian d', 'a.id=d.user_id');
