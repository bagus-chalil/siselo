...
class Ujian extends CI_Controller {
	...
	public function __construct(){
		...
		$this->siswa 	= $this->ujian->getIdSiswa($this->user->nisn);
    }
    ...
    public function akses_guru()
    {
        ...
        if ( !$data['user']['role_id'] ==2 ){
			show_error('Halaman ini khusus untuk guru untuk membuat Test Online, <a href="'.base_url('Guru').'">Kembali ke menu awal</a>', 403, 'Akses Terlarang');
		}
    }
    ...
    public function akses_siswa()
    {
        ...
        if ( !$data['user']['role_id'] ==3 ){
			show_error('Halaman ini khusus untuk siswa mengikuti ujian, <a href="'.base_url('Kelas').'">Kembali ke menu awal</a>', 403, 'Akses Terlarang');
		}
    }
	...
	public function add()
	{
        ...
        $data = [
			...
			'matpel'	=> $this->soal->getMatpelGuru(),
			'guru' => $this->ujian->getIdGuru($nip),
		];
	...
	public function list_json()
	{
		...
		$list = $this->ujian->getListUjian($this->siswa->id, $this->siswa->kelas_id);
	...
	public function list()
	{
		...
		$data = [
			...
			'title'		=> 'Ujian',
			'subjudul'	=> 'List Ujian',
			'siswa' 		=> $this->ujian->getIdSiswa($nisn),
		];
		$this->load->view('templatef/_templates/dashboard/_header.php', $data);
	...
	public function token($id)
	{
		...
		$this->load->view('templatef/topnav/_header.php', $data);
		$this->load->view('token',$data);
		$this->load->view('templatef/topnav/_footer.php');
	}
	...
	public function index()
	{
		...
		$siswa		= $this->siswa;
		$h_ujian 	= $this->ujian->HslUjian($id, $siswa->id);
